...
exports.userRequestSchema = [
    ...
    //check for correct  format
    body('name').isString().withMessage('The name must be a string'),
    body('email').isEmail().withMessage('The email must be a valid email adress'),
    body('password').isString().isStrongPassword().withMessage('The password must be at least 8 characters long and contain min 1 lowercase letter, 1 uppercase letter, 1 number and 1 symbol'),

    //check for admin value
    check('isAdmin').if(body('isAdmin').exists()).isBoolean().withMessage('The admin value must be a boolean'),
    check('isAdmin').if(body('isAdmin').exists()).custom((value, {req}) => value === false ).withMessage('The admin value must be false'),
];
